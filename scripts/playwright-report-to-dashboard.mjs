#!/usr/bin/env node
/**
 * Reads Playwright JSON report and POSTs to the realtime testing dashboard.
 *
 * If playwright-report/ exists, it is zipped and sent to
 * POST /api/ingest/github-actions/run-with-report so the HTML report appears
 * in the dashboard. GitHub artifact URLs are not public.
 */
  ...
  if (testCases.length === 0) {
    console.warn(
      '[dashboard] Parsed 0 tests from JSON. Expected suite.specs[].tests[] or suite.tests[]. ' +
        'Ensure playwright.config.ts includes ["json",{outputFile:"test-results/results.json"}].',
    );
  }

  const body = {
    suite_name: process.env.SUITE_NAME || 'Salesforce CPQ Playwright',
